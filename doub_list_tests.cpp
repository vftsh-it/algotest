...
TEST(insert, double)
{
    ...
    list_t *list = init_dll();
    int res = insert_dll (list, 0, &fst);

    ASSERT_EQ(0, res);

    res = insert_dll (list, 0, &snd);
...
TEST(insert, triple)
{
    ...
    int res = insert_dll (list, 0, &fst);
    ASSERT_EQ(0, res);
    res = insert_dll (list, 1, &snd);
    ASSERT_EQ(0, res);
    res = insert_dll (list, 2, &trd);
    ASSERT_EQ(0, res);
...
TEST(basic, set)
{
    ...
    ASSERT_EQ(0, set_dll (list, 0, &frh));
    ASSERT_EQ(0, set_dll (list, 1, &ffh));
    ASSERT_EQ(0, set_dll (list, 2, &sxh));
    ASSERT_EQ(1, set_dll (list, 3, &sxh));
...
TEST(delete, double)
{
    ...
    list_t *list = init_dll();
    insert_dll (list, 0, &fst);
    insert_dll (list, 0, &snd);
...
TEST(delete, triple)
{
    ...
    list_t *list = init_dll();
    insert_dll (list, 0, &fst);
    insert_dll (list, 1, &snd);
    insert_dll (list, 2, &trd);
...
TEST(fuzz, fuzz)
{
    ...
    for (int i = 0; i < 4'200'000; ++i) {
        ...
        int val = (1 + std::rand()) % 10'000'000;
        data.push_back(&val);

        ASSERT_EQ(0, insert_dll (list, 0, &val));
        out = get_dll (list, 0);
